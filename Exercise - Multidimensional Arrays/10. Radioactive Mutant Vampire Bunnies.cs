...
internal class Program {
    ...
    public static void Main() {
        ...
        InitializeGameObjects(matrix, playerPos, bunnies);
        ...
        for (int i = 0; !isDead && !isOutside && i < commands.Length; i++) {
            ...
            ProcessPlayerMovement(matrix, playerPos, coordinatesChanges, rows, cols, ref isDead, ref isOutside);
            if (!SpreadBunnies(matrix, bunnies)) isDead = true;
        }

        PrintMatrix(matrix);
        PrintResult(isOutside, playerPos);
    }

    private static int[] ReadDimensions() => Console.ReadLine().Split(' ', StringSplitOptions.RemoveEmptyEntries).Select(int.Parse).ToArray();

    private static void ReadMatrixData(char[,] matrix, int rows, int cols) {
    ...
    private static void InitializeGameObjects(char[,] matrix, int[] playerPos, Queue<int[]> bunnies) {
    ...
    private static void ProcessPlayerMovement(char[,] matrix, int[] playerPos, int[] coordinatesChanges, int rows, int cols, ref bool isDead, ref bool isOutside) {
    ...
    private static void PrintResult(bool isOutside, int[] playerPos) {
